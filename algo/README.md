...
木構造の一つ。プライオリティキューの実現に使用される

木の各節点に割り当てられたキーが、１つの配列（１オリジン）の各要素に対応した完全二分木  
それぞれの要素について、 i/2 で親, i*2 で左の子要素、 i\*2+1 で右の子要素にアクセスできる  

[priority queue](https://onlinejudge.u-aizu.ac.jp/status/users/matsuyoshi/submissions/1/ALDS1_9_C/judge/4157898/C++)

- 要素の追加
  - 配列末尾に要素を追加し、そこから要素とその要素の親の関係に注目して順序を整理
- 要素の取り出し
  - 常に根がルールに則って最大もしくは最小（ヒープの構成順による）
  - 根から要素を取り出したあと、配列末尾の要素を根に持ってきて再度ルールに則ったヒープ
